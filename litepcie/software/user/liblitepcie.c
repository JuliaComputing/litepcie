...
uint32_t litepcie_readl(int fd, uint32_t addr) {
    ...
    m.is_write = 0;
    m.addr = addr;
    explain_ioctl_or_die(fd, LITEPCIE_IOCTL_REG, &m);
    return m.val;
}
...
uint8_t litepcie_request_dma_reader(int fd) {
    ...
    explain_ioctl_or_die(fd, LITEPCIE_IOCTL_LOCK, &m);
                          ...
                          uint32_t tx_data)
{
    ...
    explain_ioctl_or_die(fd, LITEPCIE_IOCTL_FLASH, &m);
    flash_spi_cs(fd, 1);
    return m.rx_data;
}
